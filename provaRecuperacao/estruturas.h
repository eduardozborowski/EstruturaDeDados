...
Celula *retirarPares(Celula *lista) {
  Celula *p;
  for (p = lista; p; p = p->prox) {
     if (p->dado % 2 == 0) {
       p = excluirLista(p->dado, p);
     } 
   }
  return lista;
  }

  Celula *mediaListaFirstLast(Celula *lista) {
    Celula *p;
    Celula *pR;


    if (!lista || !lista->prox) {
      cout << "A lista não tem número mínimo de dois elementos" << endl;
    }
    
    int listaFirst = lista->dado;
    int listaTail = 0;
    for (p = lista; p->prox; p = p->prox); //levar o p ate o ultimo elemento

    listaTail = p->dado;
  
    int media = listaFirst / listaTail;
    cout << "A média é" << media << endl;
  }

void meioDaLista(Celula *lista) {
  Celula *p;
    
  int qtd = contarElementos(lista);

  if (qtd < 2) {
    cout << "Lista pequena demais" << endl;
  }
  int i;
  for (i = 0, p = lista; i < qtd / 2; i++, p = p->prox) {
    cout << p->dado << endl;
  }
